...
function main(config) {
  ...
  // SNI 嗅探：只对常见 TLS 端口嗅探，减少非加密流量的无效解析开销
  config.sniff = {
    enable: true,
    "override-destination": true,
    sniffing: ["tls", "http"],
    "port-whitelist": ["443", "8443", "8080"]
  };
  ...
  config.tun = {
    enable: true,
    // gvisor 兼容性更广，混合栈在部分 Android 设备可能不稳定
    stack: "gvisor",
    "auto-route": true,
  ...
  config.dns = {
    ...
    // geodata-mode：nameserver-policy 中 geosite 规则需要此开关
    "geodata-mode": true,
    ...
    // 默认 DNS：混入一个国外 DoH 兜底，防止未分类域名全部走国内 DNS 造成泄漏
    nameserver: [
  ...
  config.rules = [
    ...
    // GEOIP 兜底：规则集未覆盖的中国 IP 直连，减少漏网之鱼走代理
    "GEOIP,CN,DIRECT",
